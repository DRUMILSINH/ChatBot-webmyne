...
export function ChatPage() {
    const { vectorId, chats, activeChatId, messages, streamBuffer, isStreaming, debugPayload, selectedDebugMessageId, darkMode, error, setChats, addChat, setActiveChat, setMessages, addMessage, setStreaming, setStreamBuffer, appendStreamBuffer, setDebugMessage, setDebugPayload, toggleDarkMode, setError, } = useChatStore();
    ...
    const handleOpenDebug = useCallback(async (messageId) => {
    ...
    return (_jsxs(AppShell, { sidebar: _jsx(Sidebar, { chats: chats, activeChatId: activeChatId, onSelectChat: (id) => setActiveChat(id), onNewChat: () => void handleNewChat() }), children: [_jsx(TopBar, { title: chats.find((chat) => chat.id === activeChatId)?.title || "New Chat", darkMode: darkMode, onToggleTheme: toggleDarkMode }), _jsx(KnowledgeBaseExplorer, { vectorId: vectorId }), error ? _jsx(ErrorBanner, { message: error }) : null, _jsx(ChatView, { messages: messages, streamBuffer: streamBuffer, isStreaming: isStreaming, onSend: handleSend, onOpenDebug: (id) => void handleOpenDebug(id), debug: debugPayload, selectedMessageId: selectedDebugMessageId })] }));
}
